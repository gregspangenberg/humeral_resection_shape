...
fig, host = plt.subplots() # (width, height) in inches
# (see https://matplotlib.org/3.3.3/api/_as_gen/matplotlib.pyplot.subplots.html)
    
par1 = host.twinx()
par2 = host.twinx()
    
host.set_xlim(0.9, 1.1)
host.set_ylim(0, 1)
par1.set_ylim(0.8, 1)
par2.set_ylim(0,0.2)    
    
host.set_xlabel(r'$\longleftarrow Downsizing$''  |  'r'$Upsizing \longrightarrow$''     ')
host.set_ylabel("Cortical Coverage")
par1.set_ylabel("Total Coverage")
par2.set_ylabel("Overhang")
...
p1, = host.plot(datas.index, datas[['ci cort coverage']], color=color1, label="Cortical Coverage", linestyle ='dashed')
host.fill_between(datas.index, (datas[['ci cort coverage']] - datastd[['ci cort coverage']]).values.flatten(), (datas[['ci cort coverage']] + datastd[['ci cort coverage']]).values.flatten(), alpha=0.3, color=color1, linestyle ='dashed')
p1, = host.plot(datas.index, datas[['el cort coverage']], color=color1, label="Cortical Coverage")
host.fill_between(datas.index, (datas[['el cort coverage']] - datastd[['el cort coverage']]).values.flatten(), (datas[['el cort coverage']] + datastd[['el cort coverage']]).values.flatten(), alpha=0.3, color=color1)


p2, = par1.plot(datas.index, datas[['ci coverage']], color=color2, label="Total Coverage", linestyle ='dashed')
par1.fill_between(datas.index, (datas[['ci coverage']] - datastd[['ci coverage']]).values.flatten(), (datas[['ci coverage']] + datastd[['ci coverage']]).values.flatten(), alpha=0.3,color=color2, linestyle ='dashed')
p2, = par1.plot(datas.index, datas[['el coverage']], color=color2, label="Total Coverage")
par1.fill_between(datas.index, (datas[['el coverage']] - datastd[['el coverage']]).values.flatten(), (datas[['el coverage']] + datastd[['el coverage']]).values.flatten(), alpha=0.3,color=color2)



p3, = par2.plot(datas.index, datas[['ci overhang']], color=color3, label="Overhang", linestyle ='dashed')
par2.fill_between(datas.index, (datas[['ci overhang']] - datastd[['ci overhang']]).values.flatten(), (datas[['ci overhang']] + datastd[['ci overhang']]).values.flatten(), alpha=0.3, color=color3, linestyle ='dashed')
p3, = par2.plot(datas.index, datas[['el overhang']], color=color3, label="Overhang")
par2.fill_between(datas.index, (datas[['el overhang']] - datastd[['el overhang']]).values.flatten(), (datas[['el overhang']] + datastd[['el overhang']]).values.flatten(), alpha=0.3, color=color3)


dummy_cir, = plt.plot(0, 0, color='black', linestyle='dashed')
dummy_el, = plt.plot(0,0, color='black', linestyle='solid')


lns = [p1, p2, p3]
legend1 = host.legend(handles=lns, loc='best',fontsize='small')
legend2 = host.legend([dummy_el,dummy_cir],['Elliptical Head','Circular Head'], loc='lower right',fontsize='small')
host.add_artist(legend1)
host.add_artist(legend2)

# add color to axes ticks
host.tick_params(axis='y',colors=color1)
par1.tick_params(axis='y',colors=color2)
par2.tick_params(axis='y',colors=color3)


# turn all axes into percentages
host.set_xticks([0.9,0.95,1.0,1.05,1.1])
host.set_xticklabels(['-10%','-5%','0%','5%','10%'])
vals = host.get_yticks()
host.set_yticklabels(['%2.0f%%' %(i*100) for i in vals])

par1.set_yticks([0.8,0.85,0.90,0.95,1.0])
vals = par1.get_yticks()
par1.set_yticklabels(['%2.0f%%' %(i*100) for i in vals])

par2.set_yticks([0,0.05,0.1,0.15,0.2])
vals = par2.get_yticks()
par2.set_yticklabels(['%2.0f%%' %(i*100) for i in vals])
